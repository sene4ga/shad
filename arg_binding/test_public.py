...
def test_positional() -> None:
    ...
    assert bind_args(foo, 'abc', [1, 2, 3], None) \
        == dict(a='abc', b=[1, 2, 3], c=None) # pyrefly: ignore  # no-matching-overload
...
def test_varkwargs() -> None:
    ...
    assert bind_args(foo, foo=1, bar='spam', baz=['eggs']) \
        == dict(kekwargs=dict(foo=1, bar='spam', baz=['eggs'])) # pyrefly: ignore  # no-matching-overload
...
def test_kwonly_varkwargs() -> None:
    ...
    assert bind_args(foo, d=4, e=5, f=6) \
        == dict(d=4, e=5, f=6, kekwargs={}) # pyrefly: ignore  # no-matching-overload
    assert bind_args(foo, z=1, d=4, e=5, f=6) \
        == dict(d=4, e=5, f=6, kekwargs={'z': 1}) # pyrefly: ignore  # no-matching-overload
...
def test_everything() -> None:
    ...
    assert bind_args(foo, 1, 2, 3, d=4, e=5, f=6) \
        == dict(a=1, b=2, c=3, args=(), d=4, e=5, f=6, kwargs={}) # pyrefly: ignore  # no-matching-overload
    assert bind_args(foo, 1, 2, d=4, e=5) \
        == dict(a=1, b=2, c=None, args=(), d=4, e=5, f='default', kwargs={}) # pyrefly: ignore  # no-matching-overload
    assert bind_args(foo, 1, 2, 3, 4, 5, 6, d=4, e=5, z=100) \
        == dict(a=1, b=2, c=3, args=(4, 5, 6), d=4, e=5, f='default', kwargs={'z': 100}) # pyrefly: ignore  # no-matching-overload # noqa E501
    with pytest.raises(TypeError, match=ERR_MISSING_KWONLY_ARGS):
